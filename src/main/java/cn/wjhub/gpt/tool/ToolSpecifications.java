...
import static cn.wjhub.gpt.tool.JsonSchemaProperty.*;
import static cn.wjhub.gpt.tool.Utils.isNullOrBlank;
...
public class ToolSpecifications {
    ...
    public static List<ToolSpecification> toolSpecificationsFrom(Object objectWithTools) {
        Method[] declaredMethods = objectWithTools.getClass().getDeclaredMethods();
        Stream<Method> methodStream = stream(declaredMethods)
                .filter(method -> method.isAnnotationPresent(Tool.class));
        List<Method> collection = methodStream.collect(toList());
